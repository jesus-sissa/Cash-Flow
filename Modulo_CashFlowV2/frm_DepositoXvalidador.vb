...
Public Class frm_DepositoXvalidador
...
#Region "-Variables"
    ...
    'Banderas de Atasco en validadores y cartuchos
    Dim _AtascoEnCartuchoMei1 As Boolean = False
    Dim _AtascoEnCartuchoMei2 As Boolean = False
    Dim _AtascoEnApiladorMei1 As Boolean = False
    Dim _AtascoEnApiladorMei2 As Boolean = False
...
#Region "Variables Privadas"
    'controles mei
    Private meicontrol1 As New Ccash.MeiDevice
    Private meicontrol2 As New Ccash.MeiDevice
    Private BillAcceptor As Acceptor = New Acceptor()
...
#Region "Eventos Privados de la 1ra Instancia del Validador"
    ...
    Public Sub InicializarControl()
        ...
        'Crear el control
        Me.meicontrol1 = New Ccash.MeiDevice
        ...
        AddHandler BillAcceptor.OnConnected, conectarbill
    ...
    Private Sub meicontrol_OnEstadosAceptador(ByVal estadoMei As String)
        ...
        'End If
        If estadoMei.ToUpper = "ACEPTANDO" Or estadoMei.ToUpper = "INGRESANDO" Or estadoMei.ToUpper = "LISTO" Then 'New
            'para que este reinicando el contador
            _AtascoEnApiladorMei1 = False
        End If

        If estadoMei.ToUpper = "ATASCADO" Then
            lsv_Datos.BackColor = Color.Orange
            _AtascoEnApiladorMei1 = True
        End If
        ...
        If estadoMei.ToUpper = "DESCONECTADO" Then
            ...
            varPub.SegundosReceptor = 0
            varPub.SegundosSesion = 0
            'btn_Iniciar.Text = "Cajas" provisional
    ...
    ''' <remarks></remarks>
    Private Sub meicontrol_OnBilleteAceptado(ByVal billeteAceptado As Ccash.MeiDevice.DatosBilleteAceptado)
        varPub.SegundosReceptor = 0
    ...
    Private Sub Verificar()
        ...
        Dim billete As Ccash.MeiDevice.DatosBilleteEscorw = meicontrol1.BuscarBillete()
...
#Region "Eventos Privados de la segunda instancia del Validador"
    ...
    Private Sub meicontrol_OnEstadosAceptador2(ByVal estadoMei As String)
        ...
        If estadoMei.ToUpper = "ACEPTANDO" Or estadoMei.ToUpper = "INGRESANDO" Or estadoMei.ToUpper = "LISTO" Then 'New
            ...
            _AtascoEnApiladorMei2 = False
        End If

        If estadoMei.ToUpper = "ATASCADO" Then
            lsv_Datos2.BackColor = Color.Orange
            _AtascoEnApiladorMei2 = True
        End If
    ...
    ''' <remarks></remarks>
    ''' 
    Private Sub meicontrol_OnBilleteAceptado2(ByVal billeteAceptado As Ccash.MeiDevice.DatosBilleteAceptado)
    ...
    Private Sub Verificar2()
        ...
        Dim billete As Ccash.MeiDevice.DatosBilleteEscorw = meicontrol2.BuscarBillete()
...
#Region "Manejadores de Eventos"
    ...
    Private Sub tmr_depositos_Tick(sender As Object, e As EventArgs) Handles tmr_depositos.Tick
        varPub.SegundosSesion = 0
        Dim id_deposito As Integer = varPub.ID_DepositoP

#Region "Comprobacion de validadores"
        ...
        If meicontrol1.BillAcceptor.CashBoxAttached Then
            ...
            _AtascoEnCartuchoMei1 = False
            If Not _AtascoEnApiladorMei1 Then
                lsv_Datos.BackColor = Color.Green
            End If
        Else
            If varPub.Activar_Val1 Then
                lsv_Datos.BackColor = Color.Orange
                _AtascoEnCartuchoMei1 = True
                'fn_Log_Depositos("ATASCADO EN CARTUCHO 1")
            Else
                lsv_Datos.BackColor = Color.IndianRed
            End If
        ...
        'comprueba el cartucho del validador 1 2
        'CashBoxAttached True -No hay cartucho atascado
        'CashBoxAttached False -cartucho atascado
        If meicontrol2.BillAcceptor.CashBoxAttached Then
            _AtascoEnCartuchoMei2 = False
            If Not _AtascoEnApiladorMei2 Then
                lsv_Datos2.BackColor = Color.Green
            End If
        Else
            If varPub.Activar_Val2 Then
                lsv_Datos2.BackColor = Color.Orange
                _AtascoEnCartuchoMei2 = True
                'fn_Log_Depositos("ATASCADO EN CARTUCHO 2")
            Else
                lsv_Datos2.BackColor = Color.IndianRed
            End If
        End If

        If _AtascoEnCartuchoMei1 Or _AtascoEnCartuchoMei2 Then
            If Not btn_Finalizar.Enabled = False Then
                btn_Finalizar.Enabled = False
            End If
        Else
            btn_Finalizar.Enabled = True
...
#End Region
        ...
        If varPub.SegundosReceptor >= (varPub.TimeOut_Receptor) Then
            ...
            If varPub.ManejaConexionWebService = 0 Then
                ...
                'If _AtascoEnCartuchoMei1 Then
                '    AtascoMei1(id_deposito)
                'End If
                'If _AtascoEnCartuchoMei2 Then
                '    AtascoMei2(id_deposito)
                'End If
            End If
    ...
    Private Sub AtascoMei1(Id_Deposito As Integer)
        'limpiar datos de lsv_datos
        lsv_Datos.Items.Clear()
        lsv_Datos.ForeColor = Color.Red
        lbl_Total1.Text = "Total $ 0.00"
        If lsv_Datos2.Items.Count > 0 Then
            lsv_Datos2.Items.Clear()
            lsv_Datos2.ForeColor = Color.Red
            lbl_Total2.Text = "Total $ 0.00"
        Else
            lsv_Datos2.ForeColor = Color.Red
        End If
        ...
        Persistente.Cargar()
        If DenominacionV1 > 0 Then
            AnularDenominacion(Id_Deposito, DenominacionV1, 1, varPub.Serie_Val1, varPub.Serie_Caset1)
            cls_CashFlow.fn_Depositos_imprimir(1, Id_Deposito, 24)
            'Call fn_EscribirLog(varPub.UsuarioClave, "IMPRIMIR ATASCO:", DenominacionV1)
            varPub.ID_DepositoP = Id_Deposito
            Dim frm As New frm_ImprimirAtasco()
            frm.ShowDialog()
        End If

        fn_AddLog(varPub.Id_Caje, varPub.UsuarioClave, varPub.Cve_Sucursal, 16, 24, "DEPOSITOS, ID DEPOSITO: " & Id_Deposito.ToString() & " POSIBLE ATASCO DETECTADO V1." & " ULTIMA DENOMINACION ACEPTADA:  " & DenominacionV1)
        varPub.FlagImprimirTicket = 1
        varPub.ID_DepositoP = 0
        Call fn_EscribirLog(varPub.UsuarioClave, "DEPOSITOS, POSIBLE ATASCO DETECTADO V1.", " ULTIMA DENOMINACION ACEPTADA: " & DenominacionV1)
        varPub.D_On = False
    ...
    Private Sub AtascoMei2(Id_Deposito As Integer)
        'limpiar datos de lsv_datos
        lsv_Datos2.Items.Clear()
        lsv_Datos2.ForeColor = Color.Red
        lbl_Total2.Text = "Total $ 0.00"
        ...
        Persistente.Cargar()
        If DenominacionV2 > 0 Then
            AnularDenominacion(Id_Deposito, DenominacionV2, 2, varPub.Serie_Val2, varPub.Serie_Caset2)
            cls_CashFlow.fn_Depositos_imprimir(1, Id_Deposito, 24)
            varPub.ID_DepositoP = Id_Deposito
            Dim frm As New frm_ImprimirAtasco()
            frm.ShowDialog()
        End If
        fn_AddLog(varPub.Id_Caje, varPub.UsuarioClave, varPub.Cve_Sucursal, 16, 24, "DEPOSITOS, ID DEPOSITO: " & Id_Deposito.ToString() & " POSIBLE ATASCO DETECTADO V2." & " ULTIMA DENOMINACION ACEPTADA: " & DenominacionV2)
        varPub.FlagImprimirTicket = 1
        varPub.ID_DepositoP = 0
        Call fn_EscribirLog(varPub.UsuarioClave, "DEPOSITOS, POSIBLE ATASCO DETECTADO V2.", " ULTIMA DENOMINACION ACEPTADA: " & DenominacionV2)
        varPub.D_On = False
...
#Region "Eventos que gestionan Finalizar y Cancelar depósitos por WebService"
    ...
    Private Sub tmr_depositos_Disposed(sender As Object, e As EventArgs) Handles tmr_depositos.Disposed

    End Sub

    Private Sub lsv_Datos2_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lsv_Datos2.SelectedIndexChanged

    End Sub

    Private Sub lsv_Datos_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lsv_Datos.SelectedIndexChanged
